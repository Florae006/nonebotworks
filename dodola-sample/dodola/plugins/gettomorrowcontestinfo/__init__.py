...
getTomorrow=on_command(
    "tomorrow",
    aliases={"Tomorrow","next", "查询明日比赛"}, 
    priority=9,
    block=True,
)
...
async def handle_function():
    try:
        ...
        if(rescf.status_code==200):
            ...
            todaycf=[]
            today1=datetime.datetime.now()+datetime.timedelta(days=1)
            todaytime=today1.strftime('%b/%d/%Y')
            flag=0
        ...
        if(flagr==0):
            reply+="明天没有比赛哦，但是也要记得加训补题哦🧐🧐~"
        else:
            reply+=replycf+replync+replyatr
            reply+="明天的cf/牛客/Atcoder比赛如上！期待明天哦😉😉~"
        await getTomorrow.finish(str(reply))
    except Exception:
        await getTomorrow.finish()

__plugin_meta__ = PluginMetadata(
    name="getTomorrowContestInfo",
    description="",
    usage="",
    config=Config,
)
